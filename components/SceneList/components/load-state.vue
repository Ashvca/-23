    ...
    <div>
      ...
      <div style="display: flex; margin-top: 16px">
        ...
        <div style="display: flex; margin-top: 10px; margin-left: 80px">
          <span style="width: 84px">当前包数：</span>
          <FloatInput
            v-model="formData.currentTotalPackage"
            :max="8"
            :min="1"
            :maxlength="1"
            :value="formData.currentTotalPackage"
            style="width: 100px"
            class="theme-input"
          ></FloatInput>
        </div>
      ...
      <div style="margin-top: 22px; margin-left: 20px">
        <div style="display: flex">
          <span>文件内容校验和：</span>
          <FloatInput
            v-model="formData.fileContentCheckSum"
            :max="8"
            :min="1"
            :maxlength="1"
            :value="formData.fileContentCheckSum"
            style="width: 100px"
            class="theme-input"
          ></FloatInput>
        </div>
  ...
  methods: {
    submit() {
      let formData = {
        ...
        facilityValue: this.formData.facilityValue, //选择设备
        moduleId: this.formData.moduleId, //模块号
        fileTotalPackage: this.formData.fileTotalPackage, //文件总包数
        fileTotalByte: this.formData.fileTotalByte, //文件总字节数
        currentTotalPackage: this.formData.currentTotalPackage, //当前包数
        fileContentCheckSum: this.formData.fileContentCheckSum, //文件内容校验和
      }
      console.log('loadState组件表单提交')
      console.log('loadState组件表单提交 formData:')
      console.log(formData)
      console.log('loadState组件表单提交 formIndex:')
