<template>
  <div class="input-number">
    ...
    <div
      ...
      :style="{ height: line * 22 + 'vh' }"
    ></div>
    <!-- :style="{ height: line * 0.5 + 'vh' }" -->
  </div>
</template>
...
export default {
  ...
  methods: {
      ...
      if (text) {
        ...
        if (mustEle) {
          ...
          mustEle.setAttribute(
            ...
            //   this.line * 1 +
            //   'px'
            'display:block;opacity: 1;height:' + this.line * 22 + 'px'
          )
          this.errorHidden = false
          this.$emit('errorHidden', false)
        }
      }
    },
    // 隐藏错误提示
    hide() {
      ...
      if (mustEle) {
        ...
        mustEle.setAttribute(
          ...
          //   this.line * 1 +
          //   'px'
          'display:block;opacity: 0;height:' + this.line * 22 + 'px'
        )
        this.errorHidden = true
        this.$emit('errorHidden', true)
      }
    },
    // 检查步进
    checkStep() {
    ...
    // 检查错误情况并显示错误信息
    checkError() {
      this.errMsg = this.min + ' ~ ' + this.max + ' 的小数'
      if (this.decimal !== 0) {
        this.errMsg += '，最多保留' + this.decimal + '位小数'
      }
      if (this.step !== 0) {
